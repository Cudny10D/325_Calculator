...
class ModernNumberTo325Calculator:
    def __init__(self, root):
        self.root = root
        self.root.title("数字转325计算公式 - 自适应主题版")
    ...
    def update_theme(self):
        ...
        # 更新根窗口背景
        self.root.configure(bg=self.colors['bg_dark'])
        self.main_container.configure(bg=self.colors['bg_dark'])
        ...
        # 更新状态栏
        self.status_var.set(f"🎨 已切换到{('深色' if self.current_theme == 'dark' else '浅色')}主题")
    ...
    def start_theme_monitoring(self):
        ...
        def check_theme():
            ...
            if new_theme != self.current_theme:
                ...
                self.update_theme()
            self.root.after(2000, check_theme)  # 每2秒检查一次
    ...
    def create_header(self):
        ...
        # 主题切换按钮
        theme_btn = tk.Button(title_container,
                             text="🌓 切换主题",
                             font=('微软雅黑', 9),
                             fg=self.colors['text_primary'],
        ...
        subtitle_label = tk.Label(header_frame,
                                 text="将任意整数转换为包含3、2、5的数学表达式 | 自动适配系统主题",
                                 font=('微软雅黑', 10),
                                 fg=self.colors['text_secondary'],
                                 bg=self.colors['bg_dark'])
    ...
    def setup_shortcuts(self):
        ...
        self.root.bind('<Control-t>', lambda e: self.toggle_theme())  # Ctrl+T 切换主题
...
💡 提示：程序会自动检测并跟随系统主题变化（支持 macOS/Windows/Linux）"""
        
        messagebox.showinfo("使用示例", examples)
    
    def show_about(self):
        """显示关于对话框"""
        theme_status = "自动检测并同步" if HAS_DARKDETECT else "手动切换"
