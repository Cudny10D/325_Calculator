...
class ModernNumberTo325Calculator:
    def __init__(self, root):
        self.root = root
        self.root.title("数字转325计算公式 - 毛玻璃主题版")
        ...
        # 毛玻璃效果
        self.glass_effect = SimpleGlassEffect(root)
    ...
    def update_theme(self):
        ...
        glass_status = "🌫️ 半透明" if self.glass_effect.is_enabled else "✨ 标准"
        self.status_var.set(f"🎨 {theme_name}主题 | {glass_status}模式")
    ...
    def toggle_glass(self):
        """切换半透明效果"""
        if self.glass_effect.toggle():
            status = "🌫️ 半透明效果已开启" if self.glass_effect.is_enabled else "✨ 标准模式已开启"
            self.status_var.set(status)
            self.root.after(2000, lambda: self.update_theme())
        else:
            self.status_var.set("⚠️ 效果切换失败")
            self.root.after(2000, lambda: self.update_theme())
    ...
    def create_header(self):
        ...
        subtitle_label = tk.Label(header_frame,
                                 text="将任意整数转换为包含3、2、5的数学表达式 | 支持半透明效果",
                                 font=('微软雅黑', 10),
    ...
    def setup_shortcuts(self):
        ...
        self.root.bind('<Control-g>', lambda e: self.toggle_glass())
